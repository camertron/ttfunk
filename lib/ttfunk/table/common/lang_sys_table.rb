  ...
  class Table
    module Common
      class LangSysTable < TTFunk::SubTable
        ...
        attr_reader :tag, :lookup_order
        attr_reader :required_feature_index, :feature_indices
        ...
        def encode(old_to_new_features)
          EncodedString.new do |result|
            ...
            result << if includes_required_feature?
                        [old_to_new_features[required_feature_index]].pack('n')
                      else
            ...
            feature_indices.each do |feature_index|
              result << [old_to_new_features[feature_index]].pack('n')
            end
          end
        end

        def includes_required_feature?
